...
class Blockchain(object):
    ...
    def broadcast_new_block(self, new_block):
        ...
        for node in neighbors:
            r = request.post(f'http://{node}/block/new', json=data)
...
def receive_block:
    ...
    if new_block['index'] == prev_block['index'] + 1:
        # check prev hash
        if new_block['previous_hash'] == blockchain.hash(prev_block):
            # check valid proof
            block_string = json.dumps(prev_block, sort_keys=True).encode()
        ...
        # block may be invald
        # or we may be behind
        # do consensus process: ask all nodes in network for their chains and validate. choose longest valid
        return jsonify{'message': "bad block"}
